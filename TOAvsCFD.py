...
def GetLinearFit( fileName, toa, cfd ):
    ...
    for ientry, entry in enumerate(root_tree):
        if( entry.pmax2[0] > toa ):
            profile.Fill( entry.thTime2[toa]-entry.cfd3[20], entry.cfd2[cfd]-entry.cfd3[20], 1)
            hist2D.Fill( entry.thTime2[toa]-entry.cfd3[20], entry.cfd2[cfd]-entry.cfd3[20])
...
def CorrectedProjection( fileName, toa, cfd, slope, constant ):
    ...
    for ientry, entry in enumerate(root_tree):
        if( entry.pmax2[0] > toa ):
            subtract_along_cfd = getlinearY( entry.thTime2[toa]-entry.cfd3[20], slope, constant )
...
projection_canvas.SaveAs("_projection_toa{}_cfd{}.png".format(5, 60))
projection_canvas.Close()


for i in range( len(file_list) ):
    out_name = file_list[i].split("/")
    cfd_out_txt = open( "_txt_{}_{}_.txt".format(out_name[0],out_name[1]),"w")
    cfd_out_txt.write("cfd sigma")
    cfd_out_txt.write("\n")

    cfd_corr_out_txt = open( "_txt_{}_{}_corr.txt".format(out_name[0],out_name[1]),"w")
